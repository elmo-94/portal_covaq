...
class transfModel{
    ...
    public function registar(transfModel $dados){
        ...
        $query = "SELECT * FROM transf_membros WHERE id_mem=?";
        $smt = $this->Con->prepare($query);
        ...
        if($smt->rowCount() == 0){
            ...
            try {
                $query = "INSERT INTO transf_membros (data_reg, motivo, destino, localidade, estado, id_mem) 
                VALUES(?,?,?,?,?,?)";
    ...
    public function delete($id){
        ...
        try {
            $query = "DELETE FROM transf_membros WHERE id_transf =?";
            $smt = $this->Con->prepare($query);
            $smt->execute(array($id));
            return true;
        } catch (EXCEPTION $e) {
