...
class membrosModel{
    ...
    public function carregarID_Campos($id){
        ...
        try {
            ...
            id_camp,
            localidade
            from campo_missionarios";/*"SELECT
    ...
    public function registar(membrosModel $dados){
        ...
        if($smt->rowCount() == 0 OR !empty($this->data_bap) OR !empty($this->data_recep) OR strlen($this->ident) == 11){
            ...
            try {
                ...
                if(!empty($this->data_bap) || !empty($this->data_recep;)){
                    $this->registar_membrasia($this->tipo_memb, $this->data_bap, $this->data_recep, $this->ministro, $this->local_bap, $this->tempo, $id_novo, $this->proven);
                }
    ...
    public function editar(membrosModel $dados){
        ...
        $resultado = $this->Con->query("SELECT * FROM membros WHERE id_mem=$this->id_mem");  
        $resultado->execute();     
        
        try {
            
            $query = "UPDATE  membros SET  nome=?, tipo=?, identidade=?, data_val=?, genero=?, esta_civil=?,  provincia=?,  
            naturalidade=?,  data_nasc=?, idade=?, telefone=?, tel_emerg=?, email=?, pai=?, mae=?, grupo_sang=?, estado=?
            where id_mem=?";

            if($this->id_camp=='Igreja local'){
                $dados->tipo = 'Igreja local';
            }else{
                $dados->tipo = 'Campo missionário';
            }
            ...
            $dados->telefone, $dados->tel_emerg, $dados->email, $dados->pai, $dados->mae, $dados->grupo_sang, $dados->estado, $dados->id_mem)); //$dados->foto,

            //CÓDIGO PARA VERIFIAR SE EXISTE IMAGEM PARA SER ALTERADA
            if($dados->status_foto=="true"){
                $query = "UPDATE  membros SET  foto=? where id_mem=?";
                $this->Con->prepare($query)->execute(array($dados->foto, $dados->id_mem));
            }

            if(!empty($this->data_bap)){
                $this->editar_membrasia($this->id_mem);
            }
            ...
            $this->editar_formacoes($this->id_mem);
            
            if(!empty($this->id_min) && !empty($this->funcao)){

                $this->editar_ministerio($this->id_min, $this->funcao, $this->outra_funcao, $this->id_mem);

                if(!empty($this->id_min2) && !empty($this->funcao2)){

                    $this->editar_ministerio($this->id_min2, $this->funcao2, $this->outra_funcao2, $this->id_mem);

                    if(!empty($this->id_min3) && !empty($this->funcao3)){
                        
                        $this->editar_ministerio($this->id_min3, $this->funcao3, $this->outra_funcao3, $this->id_mem);
                    }
                }
            }
...
*/
    ...
    public function registar_membrasia($tipo_memb, $data_bap, $data_recep, $ministro, $local_bap, $tempo, $id_mem, $proven){

        try {
            $query = "INSERT INTO membrasias (tipo_memb, data_bap, data_recep, ministro, local_bap, tempo, id_mem, proven) 
            VALUES(?,?,?,?,?,?,?,?)";

            $this->Con->prepare($query)->execute(array($tipo_memb, $data_bap, $data_recep, $ministro, $local_bap, $tempo, $id_mem, $proven)); 
    ...
    public function editar_membrasia($id_mem){
        try {
            ...
            if($resultado->rowCount() > 0){
                ...
                where id_mem=?";
                $this->Con->prepare($query)->execute(array($this->tipo_memb, $this->data_bap, $this->data_recep, $this->ministro, $this->local_bap, $this->tempo, $this->proven, $this->id_mem)); 
            }else{
                $this->registar_membrasia($this->tipo_memb, $this->data_bap, $this->data_recep, $this->ministro, $this->local_bap, $this->tempo, $this->id_mem, $this->proven);
            }
    ...
    public function editar_formacoes($id_mem){
        try {
            $resultado = $this->Con->query("SELECT * from formacoes  WHERE id_mem=$id_mem");
            $resultado->execute();
            ...
            if($resultado->rowCount()==0){
                ...
                if(!empty($this->especialidade)){
                    $this->tipo_formacao="Acadêmica";
                    $this->registar_formacoes($this->tipo_formacao, $this->nivel, $this->instituicao, $this->especialidade, $this->id_mem, $this->outra_instituicao);
                }
                if(!empty($this->curso_teo)){
                    $this->tipo_formacao2="Teológica";
                    $this->registar_formacoes($this->tipo_formacao2, $this->nivel_teo, $this->instituicao_teo, $this->curso_teo, $this->id_mem, $this->outra_instituicao);
                }

            }else if($resultado->rowCount()>=1){            

                $query = "UPDATE  formacoes SET tipo=?, nivel=?, instituicao=?, especialidade=?, outra_instituicao=?
                where id_mem=? And id_form=?";
                if(!empty($this->especialidade)){
                    $this->tipo_formacao="Acadêmica";
                    $this->Con->prepare($query)->execute(array($this->tipo_formacao, $this->nivel, $this->instituicao, $this->especialidade, $this->outra_instituicao, $this->id_mem, $dados[0]->id_form));     
                }
                if(isset($dados[1]) && !empty($this->curso_teo)){
                    $this->tipo_formacao2="Teológica";
                    $this->Con->prepare($query)->execute(array($this->tipo_formacao2, $this->nivel_teo, $this->instituicao_teo, $this->curso_teo, $this->outra_instituicao, $this->id_mem, $dados[1]->id_form));     
                }
                //SE NÃO EXISTIR DADOS DA FORMAÇÃO TEOLÓGICA
                if(!isset($dados[1])){
                    if(!empty($this->curso_teo)){
                        $this->tipo_formacao2="Teológica";
                        $this->registar_formacoes($this->tipo_formacao2, $this->nivel_teo, $this->instituicao_teo, $this->curso_teo, $this->id_mem, $this->outra_instituicao);
    ...
    public function editar_ministerio($id_min, $funcao, $outra_funcao, $id_mem){
          
        try {
            $resultado = $this->Con->query("SELECT * FROM membros_min WHERE id_min = $id_min AND id_mem = $id_mem");
            $resultado->execute();    
            $dados = $resultado->fetch(PDO::FETCH_OBJ);
            //print_r($dados->id_min);exit;
            if($resultado->rowCount() == 0){
                $this->registar_ministerio($this->id_min, $id_mem, $this->funcao, $this->outra_funcao);
            }
            else if($resultado->rowCount() == 1){
                $query = "UPDATE  membros_min SET id_min=?, funcao=?, outra_funcao=? where id_mem=? And id_min=?";
                $this->Con->prepare($query)->execute(array($id_min, $funcao, $outra_funcao, $id_mem, $dados->id_min)); 
            }       
    ...
    public function editar_discipulador(){
        ...
        try {           
            ...
            if($resultado->rowCount() > 0){
                ...
                $query = "UPDATE  discipuladores SET data_reg=?, localidade=?, fase=?, estado=?
                where id_mem=?";
                $this->Con->prepare($query)->execute(array($this->data_reg, $this->localidade, $this->fase, $this->estado_disc, $this->id_mem));
            }
            else{
